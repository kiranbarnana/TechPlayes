...
#include "TechPlayers_version.h"
#include "TechPlayers_types.h"
...
static void TechPlayers_start(const deque<int>& data);
static void TechPlayers_end();
static void TechPlayers_file(const file_mem& fm, FILE *pfout,
...
static void usage_exit(const char *msg, const char *argv0, int code)
{
   ...
   fprintf(stdout,
           ...
           " --suffix SFX : Append SFX to the output filename. The default is '.TechPlayers'\n"
           ...
           "\n"
           "Usage Examples\n"
           "cat foo.d | TechPlayers -q -c my.cfg -l d\n"
           "TechPlayers -c my.cfg -f foo.d\n"
           "TechPlayers -c my.cfg -f foo.d -L0-2,20-23,51\n"
           "TechPlayers -c my.cfg -f foo.d -o foo.d\n"
           "TechPlayers -c my.cfg foo.d\n"
           "TechPlayers -c my.cfg --replace foo.d\n"
           "TechPlayers -c my.cfg --no-backup foo.d\n"
           "TechPlayers -c my.cfg --prefix=out -F files.txt\n"
           "\n"
...
int main(int argc, char *argv[])
{
   ...
   if (cfg_file.empty())
   {
      if (!unc_getenv("TechPlayers_CONFIG", cfg_file))
      {
         string home;
   ...
   if (!cfg_file.empty())
   {
      ...
      {
		  usage_exit(cpd.filename, argv[0], 56);
   ...
   if (cfg_file.empty())
   {
      usage_exit("Specify the config file with '-c file' or set TechPlayers_CONFIG",
                 argv[0], 58);
   }
   ...
   if ((source_file == NULL) && (source_list == NULL) && (p_arg == NULL))
   {
      ...
      TechPlayers_file(fm, stdout, parsed_file);
   }
...
const char *fix_filename(const char *filename)
{
   ...
   /* Create 'outfile.TechPlayers' */
   tmp_file = new char[strlen(filename) + 16 + 1]; /* + 1 for '\0' */
   if (tmp_file != NULL)
   {
      sprintf(tmp_file, "%s.TechPlayers", filename);
   }
   return(tmp_file);
}
                           ...
                           bool       keep_mtime)
{
   ...
   TechPlayers_file(fm, pfout, parsed_file);
