...
# extract the version from src/TechPlayers_version.h...
VERSION=`grep '#define TechPlayers_VERSION ' src/TechPlayers_version.h | \
         sed -e "s/#define TechPlayers_VERSION//" -e 's/\"//g' -e 's/ //g'`
THISDIR=$PWD
RELDIR=../release
if ! [ -e $RELDIR ] ; then
  mkdir -p $RELDIR
...
VERDIR=$RELDIR/TechPlayers-$VERSION-win32
