...
def run_tests(test_name, config_name, input_name, lang):
	...
	cmd = "%s/TechPlayers -q -c %s -f input/%s %s > %s" % (os.path.abspath('../src'), config_name, input_name, lang, resultname)
	if log_level & 2:
		print ("RUN: " + cmd)
	a = os.system(cmd)
	if a != 0:
	...
	cmd = "%s/TechPlayers -q -c %s -f %s %s > %s" % (os.path.abspath('../src'), config_name, outputname, lang, resultname)
	if log_level & 2:
		print ("RUN: " + cmd)
	a = os.system(cmd)
	if a != 0:
